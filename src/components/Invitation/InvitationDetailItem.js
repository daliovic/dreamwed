...
export default function InvitationDetailItem({ invitationDetails, setInvitationDetails, detailItem, adv, i }) {
  ...
  const resetStylesHandler = () => {
    setInvitationDetails((prev) => ({
      ...prev,
      [detailItem]: { ...prev[detailItem], styles: {} },
    }))
  }
      ...
      <motion.div className='mb-2'>
        ...
        <input
          ...
          onChange={(e) =>
            setInvitationDetails((prev) => ({
              ...prev,
              [detailItem]: { ...prev[detailItem], name: e.target.value },
            }))
          }
          ...
          <Dropdown className='col-12 me-4'>
            ...
            <Dropdown.Menu
              ...
              {STYLE_PROPERTIES.map((item, i) => {
                if (i === 0) {
                  return (
                    <Dropdown.Item disabled={true} key={item} className='border-bottom'>
                      {item}
                    </Dropdown.Item>
                  )
                }
                if (item.includes('*'))
                  return (
                    <Dropdown.Item disabled={true} key={item} className='border-bottom border-top'>
                      {item}
                    </Dropdown.Item>
                  )
                let color = ''
                  ...
                  <Dropdown.Item
                    ...
                    onClick={() => {
                      setInvitationDetails({
                        ...invitationDetails,
                        [detailItem]: {
                          ...invitationDetails[detailItem],
                          styles: {
                            ...invitationDetails[detailItem].styles,
                            currentProp: item,
                          },
                        },
                      })
          ...
          <div className='col-12 d-flex'>
            <div className='col-10 pe-4'>
              <input
                className='form-control'
                placeholder={
                  invitationDetails[detailItem].styles[invitationDetails[detailItem].styles.currentProp] &&
                  !invitationDetails[detailItem].styles.currentProp.includes('Select')
                    ? `Current Value: ${
                        invitationDetails[detailItem].styles[invitationDetails[detailItem].styles.currentProp]
                      }`
                    : '(default)'
                ...
                onBlur={(e) => {
                  setInvitationDetails({
                    ...
                    [detailItem]: {
                      ...
                      styles: {
                        ...invitationDetails[detailItem].styles,
                        [invitationDetails[detailItem].styles.currentProp]: `${e.target.value}`,
                      },
                    },
                  })
                }}
